...
function Initiative({ combatants, setCombatants, currentTurn, setCurrentTurn }) {
  ...
  useEffect(() => {
    const handleClick = (e) => {
      if (!e.target.closest('.effect-menu-wrapper')) setOpenStatusMenu(null);
    };
  ...
  useEffect(() => {
    const handleClick = (e) => {
      if (!e.target.closest('.hp-section')) setOpenHpEditor(null);
    };
    document.addEventListener('mousedown', handleClick);
    return () => document.removeEventListener('mousedown', handleClick);
  }, []);
  ...
  const nextTurn = () => {
    ...
    let updatedCombatants = combatants.map(c => {
      ...
      const updatedEffects = c.effects
        .map(e => ({ ...e, rounds: e.rounds !== null ? e.rounds - 1 : null }))
    ...
    const skippingEffect = nextCombatant?.effects.find(e => STATUS_EFFECTS[e.type]?.skipsTurn);
  ...
  const addEffect = (combatantId, effectType) => {
    setCombatants(combatants.map(c => {
      ...
      return { ...c, effects: [...c.effects, { type: effectType, rounds: STATUS_EFFECTS[effectType].defaultRounds }] };
    }));
  ...
  const applyDamage = (id, amount) => {
    setCombatants(combatants.map(c => {
      ...
      const dmgLeft   = Math.max(0, amount - tempHp);
      return { ...c, hp: Math.max(0, c.hp - dmgLeft), tempHp: newTempHp };
    }));
  };
  ...
  const applyTempHp = (id, amount) => {
    setCombatants(combatants.map(c =>
      c.id === id ? { ...c, tempHp: Math.max(c.tempHp || 0, amount) } : c
    ));
  };
  ...
  const editMaxHp = (id, newMax) => {
    setCombatants(combatants.map(c => {
      ...
      return { ...c, maxHp: clamped, hp: Math.min(c.hp, clamped) };
    }));
  };
    ...
    <div className="section section-narrow">
      ...
      <div className="add-form">
        ...
        <div className="slider-group">
          ...
          <input
            type="range" min="1" max="30" value={initiative}
        ...
        <div className="slider-group">
          <label>HP</label>
          <input
            type="number" placeholder="10" value={hp}
      ...
      {/* ---- Skip Message ---- */}
      {skipMessage && <div className="skip-message">{skipMessage}</div>}
          ...
          <div className="combatant-list">
            {combatants.map((c, index) => (
              ...
              >
                ...
                {/* Initiative badge — click to edit inline */}
                {editingInit === c.id ? (
                  <input
                      ...
                      if (e.key === 'Enter') {
                        ...
                        if (!isNaN(val)) {
                          const updated = combatants
                            .map(x => x.id === c.id ? { ...x, initiative: val } : x)
                    ...
                    onBlur={() => setEditingInit(null)}
                    autoFocus
                  />
                ) : (
                  <span
                    className="initiative-badge editable"
                    title="Click to edit initiative"
                    onClick={() => { setEditingInit(c.id); setInitInput(c.initiative); }}
                  >
                    {c.initiative}
                  </span>
                )}
                ...
                <div className="hp-section">
                  <div className="hp-display">
                    <span className="hp-current" style={{ color: getHpColor(c.hp, c.maxHp) }}>
                      {c.hp}
                    </span>
                    <span className="hp-slash">/</span>
                    <span
                      className="hp-max"
                      onClick={() => setOpenHpEditor(openHpEditor === `max-${c.id}` ? null : `max-${c.id}`)}
                    >
                      {c.maxHp}
                    </span>
                    {(c.tempHp || 0) > 0 && (
                  ...
                  <div className="hp-actions">
                    <button
                      ...
                      onClick={() => setOpenHpEditor(openHpEditor === `dmg-${c.id}` ? null : `dmg-${c.id}`)}
                      title="Deal damage"
                    >🗡️</button>
                    <button
                      className={`hp-quick-btn heal ${openHpEditor === `heal-${c.id}` ? 'active' : ''}`}
                      onClick={() => setOpenHpEditor(openHpEditor === `heal-${c.id}` ? null : `heal-${c.id}`)}
                      title="Heal"
                    >❤️</button>
                    <button
                      className={`hp-quick-btn temp ${openHpEditor === `temp-${c.id}` ? 'active' : ''}`}
                      onClick={() => setOpenHpEditor(openHpEditor === `temp-${c.id}` ? null : `temp-${c.id}`)}
                      title="Add temp HP"
                    >🛡️</button>
                  </div>
                      ...
                      <div className="hp-inline-row">
                        <input
                          ...
                          onKeyDown={e => {
                            ...
                            if (e.key === 'Escape') { setHpInput(''); setOpenHpEditor(null); }
                          }}
                          autoFocus
                        />
                        <button
                        ...
                        <button
                          ...
                          onClick={() => { setHpInput(''); setOpenHpEditor(null); }}
                        >✕</button>
                      </div>
                    </div>
                  )}
