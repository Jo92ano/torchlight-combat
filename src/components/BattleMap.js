...
// Helper: chance check - returns true X% of the time
const chance = (percent) => Math.random() * 100 < percent;

// Helper: create empty terrain grid
const emptyTerrain = () =>
  Array(ROWS).fill(null).map(() => Array(COLS).fill('empty'));

// Helper: place an obstacle into an array
...
const generateTavern = () => {
  ...
  // Chest and alchemy in back room
...
const generateForest = () => {
  ...
  // Campfire in clearing center
  if (chance(60)) placeObstacle(obstacles, 8, 8, 'fire');

  // Chest hidden in trees
...
// Map preset id to generator function
const GENERATORS = {
...
// ============================================================
// HELPER: Distance between two cells (D&D 5e diagonal rule)
// ============================================================
const calcDistance = (r1, c1, r2, c2) => {
  return Math.max(Math.abs(r1 - r2), Math.abs(c1 - c2)) * 5;
};

// ============================================================
// HELPER: Get terrain effect for a cell
// ============================================================
const getTerrainEffect = (terrain) => {
  return TERRAIN_TYPES[terrain]?.effect || null;
};

// ============================================================
// MAIN BATTLE MAP COMPONENT
// Props:
//   combatants  - list from initiative tracker
//   currentTurn - index of active combatant
// ============================================================
function BattleMap({ combatants, currentTurn }) {

  // ---- Grid terrain: 2D array of terrain type strings ----
  ...
  // ---- Is user painting (mouse held down) ----
  const isPainting = useRef(false);

  // ---- Range selection: two token ids ----
  const [rangeSelection, setRangeSelection] = useState([]);

  // ---- Dragging state ----
  const [dragging, setDragging] = useState(null);

  // ---- Show/hide generate map picker ----
  const [showGeneratePicker, setShowGeneratePicker] = useState(false);
// ---- Close generate picker when clicking outside ----
  useEffect(() => {
    const handleClick = (e) => {
  ...
  // Replaces terrain and obstacles, keeps token positions
  // ============================================================
  ...
  // ============================================================
  // RANGE SELECTION - click tokens to measure distance
  // ============================================================
  ...
  // ============================================================
  // GET TERRAIN BADGE FOR A COMBATANT (shown in initiative tracker)
  // ============================================================
